...
export default function MangaCard({ id, title, tags, authors, artists, cover, views, bookmarks }) {
    ...
    return (
        <div className="flex gap-5 shadowbox rounded-xl overflow-hidden">
            <div className="basis-1/3 flex-shrink-0 max-w-[150px] h-[235px] bg-grey">
                <Link href={`/manga/${id}`}>
                    <a>
            ...
            <div className="py-3 pr-3">
                <Link href={`/manga/${id}`}>
                    <a className="hover:text-primary transition-colors"> <h3 className="font-bold line-clamp-1 sm:line-clamp-2">{title}</h3></a>
                </Link>
