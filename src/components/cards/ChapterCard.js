...
const simpleFormat = ({title, date})=>{
    return (
        <div className="group flex py-1 w-full bg-grey px-2" style={{contentVisibility:"auto"}}>
...
function ChapterCard(props) {
    const date = dayjs(props.readableAt);
    let chapterFormat = {...props};
    
    chapterFormat.date = date.format('DD/MM/YYYY');
