...
export default function ShowMore({ children, height}) {
    ...
    return (
        <Fragment>
            ...
            {
                childrenRef.current.offsetHeight > initHeight &&
                <button className={`${state? "bg-primary active:bg-primary-dark text-dominant px-3 py-2": "bg-dominant active:bg-grey text-primary"} rounded-md mt-5 transition-all`} onClick={clickSpan}>{state? "Show more" : "Show less"}</button>
            }
        </Fragment>
    );
}